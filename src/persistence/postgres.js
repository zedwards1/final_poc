...
async function init() {
    ...
    return client.connect().then(async () => {
        ...
        await client.query('CREATE TABLE IF NOT EXISTS alarm_table (id varchar(36), name varchar(255), completed boolean)');
...
async function storeAlarm(alarm) {
    return client.query('INSERT INTO alarm_table(id, name, completed) VALUES($1, $2, $3)', [alarm.id, alarm.name, alarm.completed]).then(() => {
...
async function updateAlarm(id, alarm) {
    return client.query('UPDATE alarm_table SET name = $1, completed = $2 WHERE id = $3', [alarm.name, alarm.completed, id]).then(() => {
